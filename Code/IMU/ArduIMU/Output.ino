...
{    
 ...
 //Print Ascii messages
  ...
  Serial.print("time:");
  Serial.print(millis());  //output the software version
  Serial.print(",");
   ...
   Serial.print("\tRLLrate:\t");
   Serial.print(ToDeg(Gyro_Vector[0]));
   Serial.print("\tPCHrate:\t");
     ...
     ck=19;
		...
		for (int i=0;i<ck+2;i++) 
			...
			IMU_ck_b+=IMU_ck_a;       
                ...
                Serial.write(IMU_ck_a);
                Serial.write(IMU_ck_b);
	} //end if new GPS data 
        else 
      #endif //end #if USE_GPS
        {
       #if USE_ORIGINAL_BINARY_MESSAGE == 1
		...
		Serial.write(IMU_ck_b);
               ...
               #if PRINT_BINARY_RATES == 1 
              	// This section outputs the IMU angular rate message
                IMU_ck_a = 0;
                IMU_ck_b = 0;
		Serial.print("DIYd");  // This is the message preamble
		IMU_buffer[0]=0x06;
		...
		tempint=ToDeg(Gyro_Vector[0])*10;  //Roll rate (degrees/s) * 10 in 2 bytes
		IMU_buffer[2]=tempint&0xff;
		IMU_buffer[3]=(tempint>>8)&0xff;
      
		tempint=ToDeg(Gyro_Vector[1])*10;   //Pitch rate (degrees/s) * 10 in 2 bytes
		IMU_buffer[4]=tempint&0xff;
		IMU_buffer[5]=(tempint>>8)&0xff;
      
		templong=ToDeg(Gyro_Vector[2])*10;  //Yaw rate (degrees/s) * 10 in 2 bytes
		tempint = templong;
		IMU_buffer[6]=tempint&0xff;
		IMU_buffer[7]=(tempint>>8)&0xff;
		...
		Serial.write(IMU_ck_a);
		Serial.write(IMU_ck_b);
               #endif  // end print binary rates
            #else   // else if not using original binary message
		...
		tempint=ToDeg(Gyro_Vector[0])*50;  //Roll rate (degrees/s) * 50 in 2 bytes
		IMU_buffer[8]=tempint&0xff;
		IMU_buffer[9]=(tempint>>8)&0xff;
      
		tempint=ToDeg(Gyro_Vector[1])*50;   //Pitch rate (degrees/s) * 50 in 2 bytes
		IMU_buffer[10]=tempint&0xff;
		IMU_buffer[11]=(tempint>>8)&0xff;
      
		templong=ToDeg(Gyro_Vector[2])*50;  //Yaw rate (degrees/s) * 50 in 2 bytes
		tempint = templong;
		IMU_buffer[12]=tempint&0xff;
		IMU_buffer[13]=(tempint>>8)&0xff;
