...
#define PRINT_BINARY_RATES 1  //Will print binary rats message as well as binary attitude message
#define USE_ORIGINAL_BINARY_MESSAGE 0 // Use the original binary formatted message (only includes roll, pitch, yaw in one message)
...
// ADXL335 Sensitivity(from datasheet) => 330mV/g, 3.22mV/ADCstep => 330/3.22 = 102.48ADCstep/g
// Tested value : 101
#define GRAVITY 101 //this equivalent to 1G in the raw data coming from the accelerometer (ADCstep/G)
...
#define GRAVITY 4096  // This equivalent to 1G in the raw data coming from the accelerometer (4096ADCstep/G)
#define Accel_Scale(x) x*(GRAVITY/9.81)//Scaling the raw data of the accel to actual acceleration in meters for seconds square
...
#define Gyro_Gain_Y Gyro_Gain_X
#define Gyro_Gain_Z Gyro_Gain_X
#define Gyro_Scaled_X(x) x*ToRad(Gyro_Gain_X) //Return the scaled ADC raw data of the gyro in radians for second
#define Gyro_Scaled_Y(x) x*ToRad(Gyro_Gain_Y) //Return the scaled ADC raw data of the gyro in radians for second
#define Gyro_Scaled_Z(x) x*ToRad(Gyro_Gain_Z) //Return the scaled ADC raw data of the gyro in radians for second
#endif
...
{
  timeNow = millis();
  if((timeNow-timer)>=10)  // Main loop runs at 100Hz
  {
    timer_old = timer;
    timer = timeNow;
   ...
   #if PERFORMANCE_REPORTING == 1
    mainLoop_count++;
    if (timer-timer_old > G_Dt_max) 
      G_Dt_max = timer-timer_old;
   #endif
